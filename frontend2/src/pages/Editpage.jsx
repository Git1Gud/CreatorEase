...
import VideoDownloadButton from './DownloadButoon.jsx';
...
const handleTimeUpdate = useCallback(() => {
  if (!regionsPluginRef.current || !videoRef.current || !wavesurferRef.current) return;
  ...
  if (skipState.isSkipping) return;
  ...
  for (const region of sortedRegions) {
    const preSkipThreshold = 0.05; // 50ms before region
    const isNearRegion = currentTime >= (region.start - preSkipThreshold) && currentTime < region.start;
    const isInRegion = currentTime >= region.start && currentTime < region.end;
    
    if (isNearRegion && !skipState.fadeOutStarted) {
      // Start audio fade out for smoother transition
      setSkipState(prev => ({ ...prev, fadeOutStarted: true }));
      
      const originalVolume = vid.volume;
      const fadeSteps = 5;
      const fadeInterval = preSkipThreshold * 1000 / fadeSteps; // Distribute fade over threshold time
      
      let step = 0;
      const fadeOut = setInterval(() => {
        step++;
        vid.volume = originalVolume * (1 - step / fadeSteps);
      ...
      setSkipState(prev => ({ ...prev, isSkipping: true }));
      break;
      
    } else if (isInRegion && !skipState.isSkipping) {
      // Immediate skip if somehow we're inside a region
      setSkipState({ isSkipping: true, lastSkipTime: Date.now(), fadeOutStarted: false });
...
}, [regionsPluginRef, videoRef, wavesurferRef, skipState]);
